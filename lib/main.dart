...
// 为TV平台添加常量
const bool kIsAndroidTv = true; // 由于我们指定这个APK只会运行在TV上，因此固定为true

// TV平台全局配置
class TvConfig {
  // TV平台焦点边框颜色
  static Color focusBorderColor = Colors.blue;
  
  // TV平台焦点边框宽度
  static double focusBorderWidth = 3.0;
  
  // TV平台元素大小缩放比例
  static double elementSizeScale = 1.2;
  
  // TV平台字体大小缩放比例
  static double fontSizeScale = 1.2;
  
  // 设置是否自动全屏播放视频
  static bool autoFullScreenVideo = true;
  
  // 设置播放完成后是否自动退出
  static bool exitVideoWhenFinished = true;
}
...
void main() async {
  ...
  // 在TV平台上默认启用横屏模式
  if (kIsAndroidTv || GStorage.setting
